...
bool XBOXGamepadExtension::ButtonPressed(int gamepadBitmask, int button)
{
	...
	for (int i = 0; i < 4; i++)
	{
		if (gamepadBitmask & (1 << i))
		{
			...
			if (SDL_GameControllerGetButton(gamepad, (SDL_GameControllerButton)ConvertToPlatformButton(button))) return true;
		}
	}
    return false;
}
...
int XBOXGamepadExtension::StickLeftH(int gamepadIndex)
{
    ...
    int axis = SDL_GameControllerGetAxis(input->gamepads.at(gamepadIndex-1), SDL_CONTROLLER_AXIS_LEFTX);
...
int XBOXGamepadExtension::StickLeftV(int gamepadIndex)
{
    auto* input = dynamic_cast<SDL2InputBackend*>(Application::Instance().GetBackend()->input);
    if (!input->IsGamepadConnected(gamepadIndex-1)) return 0;
    int axis = SDL_GameControllerGetAxis(input->gamepads.at(gamepadIndex-1), SDL_CONTROLLER_AXIS_LEFTY);
    axis = (axis / 32767.0f) * 100.0f;
    if (abs(axis) < stickDeadzone) return 0;
    return axis;
}
...
int XBOXGamepadExtension::StickRightH(int gamepadIndex)
{
    auto* input = dynamic_cast<SDL2InputBackend*>(Application::Instance().GetBackend()->input);
    if (!input->IsGamepadConnected(gamepadIndex-1)) return 0;
    int axis = SDL_GameControllerGetAxis(input->gamepads.at(gamepadIndex-1), SDL_CONTROLLER_AXIS_RIGHTX);
    axis = (axis / 32767.0f) * 100.0f;
    if (abs(axis) < stickDeadzone) return 0;
    return axis;
}
...
int XBOXGamepadExtension::StickRightV(int gamepadIndex)
{
    auto* input = dynamic_cast<SDL2InputBackend*>(Application::Instance().GetBackend()->input);
    if (!input->IsGamepadConnected(gamepadIndex-1)) return 0;
    int axis = SDL_GameControllerGetAxis(input->gamepads.at(gamepadIndex-1), SDL_CONTROLLER_AXIS_RIGHTY);
    axis = (axis / 32767.0f) * 100.0f;
    if (abs(axis) < stickDeadzone) return 0;
    return axis;
}
...
int XBOXGamepadExtension::TriggerLeft(int gamepadIndex)
{
    auto* input = dynamic_cast<SDL2InputBackend*>(Application::Instance().GetBackend()->input);
    if (!input->IsGamepadConnected(gamepadIndex-1)) return 0;
    int axis = SDL_GameControllerGetAxis(input->gamepads.at(gamepadIndex-1), SDL_CONTROLLER_AXIS_TRIGGERLEFT);
    axis = (axis / 32767.0f) * 100.0f;
...
int XBOXGamepadExtension::TriggerRight(int gamepadIndex)
{
    auto* input = dynamic_cast<SDL2InputBackend*>(Application::Instance().GetBackend()->input);
    if (!input->IsGamepadConnected(gamepadIndex-1)) return 0;
    int axis = SDL_GameControllerGetAxis(input->gamepads.at(gamepadIndex-1), SDL_CONTROLLER_AXIS_TRIGGERRIGHT);
    axis = (axis / 32767.0f) * 100.0f;
    if (abs(axis) < triggerDeadzone) return 0;
    return axis;
}
...
void XBOXGamepadExtension::Vibrate(int gamepadBitmask, int leftMotor, int rightMotor, unsigned int duration)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        if (gamepadBitmask & (1 << i))
        {
            ...
            SDL_GameController* gamepad = input->gamepads.at(i);
            SDL_GameControllerRumble(gamepad, left, right, duration);
        }
    }
}
